...
class Bancos extends CI_Controller {
    ...
    public function asignacion_security(){ //si podemos hacer la asignacion mas generica, le cambiamos el nombre por asignacion_ftp_masiva
       $this->init();
       ...
       $nuevo_log = array(
        ...
        foreach($extrae_archivo_ftp as $a){
            if($a['correcto']){
                $id_extraccion = $a['id_extraccion'];
                $tabla_truncar = $this->MyModel->buscar_model('extraccion',array('id_extraccion' => $id_extraccion));
                $tabla = $tabla_truncar[0]['tabla'];
                $this->MyModel->truncar($tabla);
                $this->MyModel->carga_archivo_base($a['archivo'],$tabla,'*');
                $update_log = array('observacion' => 'Cargado Archivo '.$a['archivo'].'En la tabla'.$tabla);
                $this->MyModel->agregar_model('log',$update_log,$id_log);
        ...
        /*
        CUADRATURA ???????????????? NO SE QUE ENVIAR XD
        ...
        $update_log = array('observacion' => 'Call procedimiento');
        $this->MyModel->agregar_model('log',$update_log,$id_log);
        $this->MyModel->call_function($proceso[0]['procedimiento_almacenado'])or die(mysql_error());
        $update_log = array(
                'finalizo_exito' => 'S',
    ...
    public function cuadratura_security(){
